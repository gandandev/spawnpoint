...
export async function build262() {
  ...
  html = html.replace(anchor, `<script>window.spawnpointPreviewCloud=${process.env.PREVIEW_CLOUD === 'true'};</script><script src="/profile-26.2.js"></script><script>if(new URLSearchParams(location.search).has("launch"))document.write('<script src="portal-bridge-26.2.js"><\\/script>');</script><script src="client-26.2.js"></script>\n` + anchor);
  ...
  if (html.split(serverCompile).length !== 2) throw Error('Server worker compilation anchor changed');
  html = html.replace(serverCompile, 'const serverModulePromise = !new URLSearchParams(location.search).has("launch") ?');
