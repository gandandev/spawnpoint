...
**Compatibility limit:** the older JS model/muting hooks, actual-frame measurement hook and client torch shader described above belong to the retained 26.1.2 comparison client. They are not applied to the new Wasm runtime. The 26.2 client applies held-light distance attenuation to terrain, items, and entities through its WebGL shader hook. Model vertices are converted from camera-relative to world coordinates using the built-in Globals buffer; orthographic GUI previews keep their original light. No server lighting plugin is installed or enabled. Target-device 60/120 FPS and live GUI resize remain unverified.
