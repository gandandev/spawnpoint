...
export function Logo() {
  return <div className="flex shrink-0 items-center gap-3 font-mono text-sm font-bold tracking-tight" aria-label="spawnpoint">
    ...
    <span className="max-[359px]:hidden">spawnpoint</span>
  </div>;
}
...
export function ServerCard({ status, setupReady, onStart, compact = false, showPlayerDropdown = false }: ServerCardProps) {
  ...
  if (compact) return <Card size="sm" className={cn("t-acc relative gap-0 overflow-hidden border-0 bg-muted py-0 shadow-none ring-0", status.phase === "online" && "bg-[#96ce4d]/15", canExpand && "transition-colors hover:bg-[#96ce4d]/25 focus-within:bg-[#96ce4d]/25")} data-open={expanded}>
    <div className="relative flex min-h-11 items-center gap-3 px-2 pl-3.5">
      ...
      {canExpand ? <button
        type="button"
        className="t-acc-head relative ml-auto flex min-h-9 touch-manipulation cursor-pointer items-center gap-1.5 rounded-lg px-2 text-sm font-medium text-[#65952c] max-sm:min-h-11 [@media(max-height:480px)]:min-h-11 [@media(pointer:coarse)]:min-h-11 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#65952c]/30"
      ...
      </button> : status.phase === "online" ? label.detail && <span className="relative ml-auto mr-2 text-sm text-[#65952c]">{label.detail}</span> : onStart && <span className="relative ml-auto"><StartButton status={status} setupReady={setupReady} onStart={onStart} /></span>}
    </div>
    {canExpand && <div id="online-player-list" className="t-acc-panel" aria-hidden={!expanded}><div className="t-acc-panel-inner"><div className="px-3.5 py-2">
      {loadingPlayers && players.length === 0 ? <span className="flex items-center gap-2 text-xs text-[#65952c]"><Spinner />접속자 확인 중</span> : <ul className="flex flex-col gap-1.5" aria-label="현재 플레이 중인 사람">
        {players.map((player) => <li key={player.gameUsername} className="flex items-center gap-2 text-sm text-[#4f7622]" title={player.gameUsername}><Circle className="size-2 fill-current" />{player.displayName}</li>)}
      </ul>}
    </div></div></div>}
  </Card>;
