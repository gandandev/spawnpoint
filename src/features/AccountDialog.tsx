...
export function AccountDialog({ data, onSession, notice }: AccountDialogProps) {
  ...
  const changePassword = async (event: FormEvent) => {
    ...
    try {
      const result = await api<SessionUpdate>("/account/password", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-spawnpoint-csrf": data.csrf! },
    ...
    <DialogContent className="max-h-[calc(100dvh-1rem)] overflow-y-auto ring-0 sm:max-w-md">
      ...
      <form className="flex flex-col gap-4" onSubmit={changePassword}>
        ...
        <FieldGroup className="gap-2">
          ...
          <Field><FieldLabel className="sr-only" htmlFor="new-password">새 비밀번호</FieldLabel><Input id="new-password" type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder="새 비밀번호 (8자 이상)" autoComplete="new-password" minLength={8} maxLength={128} required /></Field>
