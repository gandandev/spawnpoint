  ...
  return <div className="admin-player-detail">
    ...
    <div className="flex min-w-0 flex-wrap content-center items-start gap-2">
      ...
      <div className="grid w-full grid-cols-2 gap-1.5 sm:flex sm:w-auto sm:flex-wrap">
        <Button type="button" className="min-w-0 px-2" variant={player?.operator ? "destructive" : "outline"} size="sm" disabled={isBusy(`op:${playerId}`)} onClick={() => void run(`op:${playerId}`, `/admin/players/${playerId}/operator`, "PUT", { operator: !player?.operator }, player?.operator ? "OP를 회수했어요." : "OP를 부여했어요.")}>{isBusy(`op:${playerId}`) ? <Spinner /> : <Shield />}{player?.operator ? "OP 회수" : "OP 부여"}</Button>
        <Button type="button" className="min-w-0 px-2" variant="outline" size="sm" disabled={!player?.online || isBusy(`kick:${playerId}`)} onClick={() => void run(`kick:${playerId}`, `/admin/players/${playerId}/kick`, "POST", { reason }, "플레이어를 내보냈어요.")}><Kick />킥</Button>
        <Button type="button" className="min-w-0 px-2" variant={player?.banned ? "outline" : "destructive"} size="sm" disabled={isBusy(`ban:${playerId}`)} onClick={() => void run(`ban:${playerId}`, `/admin/players/${playerId}/ban`, "PUT", { banned: !player?.banned, reason }, player?.banned ? "차단을 풀었어요." : "플레이어를 차단했어요.")}><Ban />{player?.banned ? "밴 해제" : "밴"}</Button>
        <Button type="button" className="min-w-0 px-2" variant="outline" size="sm" title={user.archivedAt === null && user.id === currentUserId ? "내 계정은 보관할 수 없어요." : user.archivedAt === null && player?.online ? "게임에 접속 중인 사용자는 보관할 수 없어요." : undefined} disabled={isBusy(`archive:${playerId}`) || (user.archivedAt === null && (user.id === currentUserId || player?.online))} onClick={() => void onArchive(user.archivedAt === null)}>{isBusy(`archive:${playerId}`) ? <Spinner /> : user.archivedAt !== null ? <ArchiveRestore /> : <Archive />}{user.archivedAt !== null ? "보관 해제" : "보관"}</Button>
      </div>
